...
const OTPVerification = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`https://avantgardefinance-api.onrender.com/verify/${userId}`, {
        userInput: otpValue,
      });
  ...
  const resendOTP = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`https://avantgardefinance-api.onrender.com/resend-otp/${userId}`);
      const { message } = response.data;
      toast.success(message);
    } catch (error) {
      if (error.response) {
